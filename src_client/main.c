...
int main(int argc, char **argv)
{
    ...
    while (param->running)
    {
        if (env.conn.ok && poll(&env.conn.pollfd, 1, -1) < 0)
            return perror("Poll"), 1;
        for (int i = 0; i < env.conn.ok; i++) {
            if (env.conn.pollfd.revents & POLLIN) {
                if (i == 0) {
        ...
        while (env.param.state != FINISHED && to_parse(env.conn.buffer, env.conn.buffer_len))
        {
            ...
            case PLAYING: if (handle_play(env.conn.buffer, &env.conn.buffer_len, &env.map, param, &env.conn) < 0) return EXIT_FAILURE; break;
            case FINISHED: break;
            default: break;
            }
        }
        ...
        if (env.param.state == FINISHED)
            display_finish(env.sdl.renderer, param);
    }
